...
describe(`[${env.DB_TYPE}] havingRelated (bigint)`, () => {
  test("base case: has at least one related post", async () => {
    const [u1, u2] = await UserFactory.userWithBigint(sql, 2);
    await PostFactory.postWithBigint(sql, u1.id, 1);
    const noisyUsers = await UserFactory.userWithBigint(sql, 2);
    for (const nu of noisyUsers) {
      await PostFactory.postWithBigint(sql, nu.id, 2);
    }
  ...
  test("with callback: has post with specific title", async () => {
    const [u1, u2] = await UserFactory.userWithBigint(sql, 2);
    const p = await PostFactory.postWithBigint(sql, u1.id, 1);
    await PostFactory.postWithBigint(sql, u1.id, 1);
    const noisyUsers = await UserFactory.userWithBigint(sql, 2);
    for (const nu of noisyUsers) {
      await PostFactory.postWithBigint(sql, nu.id, 2);
  ...
  test("with operator/value: more than N posts", async () => {
    const [u1, u2] = await UserFactory.userWithBigint(sql, 2);
    await PostFactory.postWithBigint(sql, u1.id, 6);
    await PostFactory.postWithBigint(sql, u2.id, 2);
    const u3 = await UserFactory.userWithBigint(sql, 1);
    await PostFactory.postWithBigint(sql, u3.id, 5);
  ...
  test("orHavingRelated: returns users with posts even if prior where filters none", async () => {
    const [u1] = await UserFactory.userWithBigint(sql, 2);
    await PostFactory.postWithBigint(sql, u1.id, 1);
    const users = await sql
      .from(UserWithBigint)
      .where("id", "=", -1)
  ...
  test("notHavingRelated: returns users without posts", async () => {
    const [u1, u2] = await UserFactory.userWithBigint(sql, 2);
    await PostFactory.postWithBigint(sql, u2.id, 1);
    const users = await sql
      .from(UserWithBigint)
      .notHavingRelated("posts")
      .many();
    const ids = users.map((u) => u.id);
    expect(ids).toContain(u1.id);
    expect(ids).not.toContain(u2.id);
  ...
  test("hasOne: user has a single post", async () => {
    ...
    await PostFactory.postWithBigint(sql, u1.id, 1);
    const u3 = await UserFactory.userWithBigint(sql, 1);
    await PostFactory.postWithBigint(sql, u3.id, 1);
  ...
  test("manyToMany base case: user has addresses", async () => {
    ...
    for (const addr of addresses) {
      await UserAddressFactory.userAddressWithBigint(sql, 1, u1.id, addr.id);
    }

    const u3 = await UserFactory.userWithBigint(sql, 1);
    const noiseAddrs = await AddressFactory.addressWithBigint(sql, 2);
    for (const addr of noiseAddrs) {
      await UserAddressFactory.userAddressWithBigint(sql, 1, u3.id, addr.id);
    }
  ...
  test("manyToMany with operator/value: more than N addresses", async () => {
    const [u1, u2] = await UserFactory.userWithBigint(sql, 2);
    const addrsU1 = await AddressFactory.addressWithBigint(sql, 6);
    const addrsU2 = await AddressFactory.addressWithBigint(sql, 2);
    for (const a of addrsU1) {
      await UserAddressFactory.userAddressWithBigint(sql, 1, u1.id, a.id);
    }
    for (const a of addrsU2) {
      await UserAddressFactory.userAddressWithBigint(sql, 1, u2.id, a.id);
    }
    const u3 = await UserFactory.userWithBigint(sql, 1);
    const addrsU3 = await AddressFactory.addressWithBigint(sql, 5);
    for (const a of addrsU3) {
      await UserAddressFactory.userAddressWithBigint(sql, 1, u3.id, a.id);
    }
  ...
  test("belongsTo with callback: posts with specific user", async () => {
    const [u1, u2] = await UserFactory.userWithBigint(sql, 2);
    await PostFactory.postWithBigint(sql, u1.id, 3);
    await PostFactory.postWithBigint(sql, u2.id, 1);
