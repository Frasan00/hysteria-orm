...
async function getAbsolutePath(migrationModulePath) {
...
async function findMigrationModule(migrationName, migrationModulePath = process.env.MIGRATION_PATH ? process.env.MIGRATION_PATH + "/" + migrationName : "database/migrations/" + migrationName) {
  ...
  const parentPath = import_path2.default.resolve(import_path2.default.dirname(absolutePath), "..");
  if (parentPath === import_path2.default.dirname(absolutePath)) {
    throw new Error(
  ...
  return findMigrationModule(
    migrationName,
    import_path2.default.join(parentPath, import_path2.default.basename(migrationModulePath))
  );
}
...
if (arg2 && !arg2.endsWith(".ts")) {
  arg2 += ".ts";
}
runMigrationsConnector(arg2).then(() => {
  process.exit(0);
}).catch((error) => {
...
if (arg3 && !arg3.endsWith(".ts")) {
  arg3 += ".ts";
}
rollbackMigrationConnector(arg3).then(() => {
  logger_default.info("migrations rolled back successfully");
  process.exit(0);
}).catch((error) => {
  logger_default.error(error);
  process.exit(1);
