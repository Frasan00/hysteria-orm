...
async function getMigrations(tsconfigPath) {
...
async function loadMigrationModule(pathToFile, tsconfigPath) {
  const isTs = pathToFile.endsWith(".ts");
  if (isTs) {
    const a = (0, import_ts_node.register)({
      transpileOnly: true,
      project: tsconfigPath
    });
    console.log(a);
    const migrationModule2 = require2(pathToFile);
    return migrationModule2.default;
  }
  const migrationModule = await import(pathToFile);
  return migrationModule.default;
}
async function findMigrationModule(migrationName, tsconfigPath, migrationModulePath = process.env.MIGRATION_PATH ? process.env.MIGRATION_PATH + "/" + migrationName : "database/migrations/" + migrationName) {
...
function findMigrationNames() {
  ...
  const migrationPath = import_node_path.default.resolve(
    process.env.MIGRATION_PATH || "database/migrations"
  );
  const fullPathToMigrationPath = import_node_path.default.resolve(
    currentUserDirectory,
    migrationPath
  );
  try {
    const migrationFiles = import_node_fs.default.readdirSync(fullPathToMigrationPath);
...
async function runMigrationsPg(runUntil, tsconfigPath) {
  ...
  try {
    ...
    const migrations = await getMigrations(tsconfigPath);
...
async function runMigrationsSql(runUntil, tsconfigPath) {
  const sql = await SqlDataSource.connect();
  const sqlConnection = sql.getCurrentConnection();
  try {
    log(BEGIN_TRANSACTION, true);
...
async function runMigrationsSQLite(runUntil, tsconfigPath) {
  const sql = await SqlDataSource.connect();
  const sqlConnection = sql.getCurrentConnection();
  try {
    const migrationTable = await getMigrationTable(sqlConnection) || [];
...
async function runMigrationsConnector(runUntil, tsconfigPath) {
...
async function migrationRollBackSql(rollBackUntil, tsconfigPath) {
  const sql = await SqlDataSource.connect();
  const sqlConnection = sql.getCurrentConnection();
  try {
    log(BEGIN_TRANSACTION, true);
...
async function migrationRollBackPg(rollBackUntil, tsconfigPath) {
  const sql = await SqlDataSource.connect();
  const sqlConnection = sql.getCurrentConnection();
  try {
    const migrationTable = await getMigrationTable(sqlConnection);
...
async function migrationRollBackSqlite(rollBackUntil, tsconfigPath) {
  const sql = await SqlDataSource.connect();
  const sqlConnection = sql.getCurrentConnection();
  try {
    const migrationTable = await getMigrationTable(sqlConnection) || [];
...
async function rollbackMigrationConnector(rollBackUntil, tsconfigPath) {
  const databaseType = process.env.DB_TYPE;
...
program.command("run:migrations [runUntil]").option("-t, --tsconfig [path]", "Path to tsconfig.json file", void 0).description(
  "Run pending migrations, if runUntil is provided, it will run all migrations until the provided migration name"
).action(async (runUntil, option) => {
  await runMigrationsConnector(runUntil, option?.tsconfig);
});
program.command("rollback:migrations [rollbackUntil]").option("-t, --tsconfig [path]", "Path to tsconfig.json file", void 0).description(
...
).action(async (rollbackUntil, option) => {
  await rollbackMigrationConnector(rollbackUntil, option?.tsconfig);
});
program.command("refresh:migrations").option("-t, --tsconfig [path]", "Path to tsconfig.json file", void 0).description(
  "Rollbacks every migration that has been run and then run the migrations"
).action(async (option) => {
  await rollbackMigrationConnector(void 0, option?.tsconfig);
  await runMigrationsConnector(void 0, option?.tsconfig);
});
