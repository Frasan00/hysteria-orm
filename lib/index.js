...
function dynamicColumn(columnName) {
  return (target, propertyKey) => {
    ...
    console.log("dynamicColumn", dynamicColumn2);
...
var MysqlQueryBuilder = class _MysqlQueryBuilder extends QueryBuilder {
  ...
  mergeQueryBuilder(queryBuilder) {
  ...
  mergeRawPacketIntoModel(model, row, typeofModel) {
    ...
    this.dynamicColumns.map((dynamicColumn2) => {
...
__decorateClass([
  column()
], User.prototype, "signupSource", 2);
__decorateClass([
  column({ booleanColumn: true })
], User.prototype, "isActive", 2);
__decorateClass([
  column()
], User.prototype, "json", 2);
__decorateClass([
  column()
...
(async () => {
  ...
  const model = await User.query().addDynamicColumns(["getTest"]).one();
  console.log(model);
