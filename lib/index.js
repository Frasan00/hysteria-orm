...
var COLUMN_METADATA_KEY2 = Symbol("columns");
var DYNAMIC_COLUMN_METADATA_KEY2 = Symbol("dynamicColumns");
function getMongoModelColumns(target) {
  return Reflect.getMetadata(COLUMN_METADATA_KEY2, target.prototype) || [];
}
...
async function serializeMongoModel(model, data, dynamicColumnsToAdd = ["test"]) {
  if (!data) {
    return null;
  }
...
var _MongoDataSource = class _MongoDataSource extends DataSource {
  ...
  static async connect(url, options, cb) {
    ...
    this.instance.logs = options?.logs || false;
    cb?.();
