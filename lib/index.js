...
var Model = class extends AbstractModel {
  ...
  static async softDelete(modelsqlInstance, options) {
    ...
    if (typeof value === "string") {
      modelsqlInstance[column2] = import_luxon5.DateTime.fromISO(
        value
      );
    }
    modelsqlInstance[column2] = value;
...
// src/no_sql/mongo/mongo_models/mongo_model_types.ts
function getBaseCollectionName(target) {
  const className = target.name;
  return className.endsWith("s") ? convertCase(className, "snake") : convertCase(className, "snake") + "s";
}

// src/no_sql/mongo/mongo_models/mongo_model.ts
var collectionMap = /* @__PURE__ */ new WeakMap();
function getBaseMongoModelInstance() {
  return { extraColumns: {} };
}
var MongoModel = class extends AbstractModel {
  /**
  ...
  static dispatchModelManager(options) {
    ...
    if (options.session) {
      return options.session.mongoDataSource.getModelManager(
        this,
...
__decorateClass([
  column()
], MongoModel.prototype, "id", 2);

// src/no_sql/mongo/mongo_models/mongo_model_decorators.ts
var MONGO_COLUMN_METADATA_KEY = Symbol("mongoColumns");
var DYNAMIC_COLUMN_METADATA_KEY2 = Symbol("dynamicColumns");
function mongoColumn() {
  return (target, propertyKey) => {
    const columns = Reflect.getMetadata(MONGO_COLUMN_METADATA_KEY, target) || [];
    columns.push(propertyKey);
    Reflect.defineMetadata(MONGO_COLUMN_METADATA_KEY, columns, target);
  };
}
function getMongoModelColumns(target) {
  return Reflect.getMetadata(MONGO_COLUMN_METADATA_KEY, target.prototype) || [];
}
function getMongoDynamicColumns(target) {
  return Reflect.getMetadata(DYNAMIC_COLUMN_METADATA_KEY2, target.prototype);
}

// src/no_sql/mongo/mongo_serializer.ts
async function serializeMongoModel(model, data, selectedColumns = [], dynamicColumnsToAdd = []) {
  if (!data) {
    return null;
  }
  const serializedModel = getBaseMongoModelInstance();
...
async function serializeMongoModels(model, data, selectedColumns = [], dynamicColumnsToAdd = []) {
...
var MongoQueryBuilder = class {
  constructor(model, mongoDataSource, logs) {
    this.model = model;
    this.dynamicColumns = [];
    this.whereObject = {};
    this.groupByObject = {};
  ...
  async many(options = {}) {
    ...
    const result = await this.collection.find(this.whereObject, { projection: this.selectFields }).toArray();
  ...
  convertIdToObjectId(searchObject) {
    if (searchObject.id) {
...
// src/no_sql/mongo/mongo_models/mongo_model_manager.ts
var MongoModelManager = class {
  constructor(mongoModel, mongoDataSource, logs = false) {
    this.logs = logs;
    this.mongoDataSource = mongoDataSource;
  ...
  async insert(modelData) {
    ...
    return await serializeMongoModel(this.model, insertedDocument);
  }
  async insertMany(modelData) {
    const result = await this.collection.insertMany(
      modelData
    );
    const insertedIds = result.insertedIds;
    const insertedDocuments = await this.collection.find({
      _id: { $in: Object.values(insertedIds) }
    }).toArray();
    return await Promise.all(
      insertedDocuments.map(async (document) => {
        return await serializeMongoModel(this.model, document);
      })
    );
  }
};
...
var _MongoDataSource = class _MongoDataSource extends DataSource {
  ...
  getModelManager(model, mongoDataSource) {
    return new MongoModelManager(model, mongoDataSource, this.logs);
  }
};
...
// src/index.ts
var User = class extends MongoModel {
  getTest() {
    return "test";
  }
};
