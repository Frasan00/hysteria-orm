...
var ModelUpdateQueryBuilder = class extends WhereQueryBuilder {
   ...
   * @returns
   */
  async getBeforeUpdateQueryIds(sqlConnection) {
    const beforeUpdateData = await sqlConnection.query(
  ...
  async getAfterUpdateQuery(sqlConnection, modelIds) {
    const afterUpdateDataQuery = `SELECT * FROM ${this.table} ${this.joinQuery} WHERE ${this.model.primaryKey} IN (${Array(modelIds.length).fill("?").join(",")})`;
    log(afterUpdateDataQuery, this.logs, modelIds);
    const updatedData = await sqlConnection.query(
...
var MysqlUpdateQueryBuilder = class _MysqlUpdateQueryBuilder extends ModelUpdateQueryBuilder {
  ...
  async withData(data, trx) {
    ...
    try {
      ...
      const data2 = await parseDatabaseDataIntoModelResponse(
        updatedData,
        this.model
      );
...
var MysqlTransaction = class {
  ...
  async massiveUpdateQuery(query, params, selectQueryDetails) {
    if (!this.mysql) {
      throw new Error("MysqlTransaction not started.");
    }
    const { typeofModel, modelIds, table, joinClause, primaryKey } = selectQueryDetails;
    try {
      log(query, this.logs, params);
      ...
      const updatedData = await this.mysql.query(
        afterUpdateDataQuery,
        [modelIds]
      );
      return await parseDatabaseDataIntoModelResponse(
  ...
  async massiveDeleteQuery(query, params, models, typeofModel) {
    if (!this.mysql) {
      throw new Error("MysqlTransaction not started.");
    }
    log(query, this.logs, params);
...
var SQLiteQueryBuilder = class _SQLiteQueryBuilder extends QueryBuilder {
  ...
  async getCount(options = { ignoreHooks: false }) {
    if (!options.ignoreHooks) {
      const result2 = this.promisifyQuery(
        "SELECT COUNT(*) as total FROM " + this.table,
        []
      );
      return +result2[0].total;
...
var SQLiteModelManager = class extends AbstractModelManager {
  ...
  async updateRecord(model, trx) {
    ...
    try {
      ...
      const selectQuery = SELECT_default("sqlite", this.model).selectAll;
      return await this.promisifyQuery(selectQuery, [
        model[this.model.primaryKey]
      ]);
    } catch (error) {
      queryError(error);
...
var SQLiteTransaction = class {
  ...
  promisifyQuery(query, params) {
    return new Promise((resolve, reject) => {
      this.sqLite.get(query, params, (err, result) => {
        if (err) {
          reject(err);
        }
        resolve(result);
...
(async () => {
  ...
  const user = await User.query().one();
  user.isActive = 0;
  const users = await User.deleteQuery().softDelete({
    column: "deletedAt",
    value: import_luxon6.DateTime.local().toISODate()
  });
  console.log(users);
