...
function queryError(error2) {
  logger.error("Query Failed ", error2);
}
  ...
  return {
    ...
    selectColumns: (...columns) => {
      columns = columns.map((column2) => {
        if (commonSelectMethods.includes(column2.toUpperCase()) || column2.includes("(")) {
          return column2;
        }
...
var QueryBuilder = class {
  ...
  async mergeRawPacketIntoModel(model, row, typeofModel) {
    ...
    const promises = this.dynamicColumns.map(async (dynamicColumn2) => {
...
var insertTemplate = (dbType, typeofModel) => {
  ...
  return {
    insert: (columns, values) => {
      ...
      const query = dbType === "mysql" || dbType === "mariadb" ? `INSERT INTO ${table} (${columns.join(", ")})
...
var SqlModelManagerUtils = class {
  ...
  async parseQueryBuilderRelations(models, typeofModel, input, logs) {
    ...
    try {
      ...
      (0, import_console.log)(relationQuery, logs);
      ...
      queryError("Query Error: " + relationQuery + error2);
      throw new Error("Failed to parse relations " + error2);
    }
  }
...
var MysqlQueryBuilder = class _MysqlQueryBuilder extends QueryBuilder {
  ...
  async one(options = { throwErrorOnNull: false }) {
    ...
    try {
      ...
      return (await this.model.afterFetch([model]))[0];
    } catch (error2) {
      queryError(query);
      throw new Error("Query failed " + error2);
    }
  }
  async oneOrFail() {
...
var MysqlDeleteQueryBuilder = class _MysqlDeleteQueryBuilder extends ModelDeleteQueryBuilder {
  ...
  async softDelete(options) {
    const {
      ...
      value = import_luxon2.DateTime.local().toString(),
      trx
    } = options || {};
...
var MysqlModelManager = class extends AbstractModelManager {
   ...
   */
  async deleteByColumn(column2, value, trx) {
    if (trx) {
      ...
      return await trx.queryDelete(query, params);
    }
    try {
      const { query, params } = this.sqlModelManagerUtils.parseDelete(
        this.model.table,
  ...
  async deleteRecord(model, trx) {
    try {
      if (!this.model.primaryKey) {
        throw new Error(
          "Model " + this.model.table + " has no primary key to be deleted from, try deleteByColumn"
        );
      }
      const { query, params } = this.sqlModelManagerUtils.parseDelete(
        this.model.table,
      ...
      log(query, this.logs, params);
      await this.mysqlConnection.query(query, params);
      return model;
    } catch (error2) {
      queryError(error2);
      throw new Error("Query failed " + error2);
...
var PostgresQueryBuilder = class _PostgresQueryBuilder extends QueryBuilder {
  ...
  async many() {
    ...
    try {
      ...
      throw new Error("Query failed: " + error2.message);
    }
  }
  async getCount() {
    this.select("COUNT(*) as total");
...
var PostgresDeleteQueryBuilder = class _PostgresDeleteQueryBuilder extends ModelDeleteQueryBuilder {
  ...
  async softDelete(options) {
    const {
      column: column2 = "deletedAt",
      value = import_luxon3.DateTime.local().toString(),
      trx
    } = options || {};
    let { query, params } = this.updateTemplate.massiveUpdate(
      [column2],
...
var MysqlTransaction = class {
  ...
  async massiveInsertQuery(query, params, typeofModel) {
    ...
    try {
      ...
      queryError(error2);
      throw new Error(
        "Failed to execute massive insert query in transaction " + error2
      );
    }
  }
  ...
  async start() {
    try {
      ...
      queryError(error2);
      throw new Error("Failed to start transaction " + error2);
    }
  }
  /**
   * Commit transaction.
  ...
  async commit() {
    ...
    try {
      ...
      queryError(error2);
      throw new Error("Failed to commit transaction " + error2);
    }
  }
  /**
  ...
  async rollback() {
    ...
    try {
      ...
      queryError(error2);
      throw new Error("Failed to rollback transaction " + error2);
    }
  }
};
...
var PostgresTransaction = class {
  ...
  async queryInsert(query, params, typeofModel) {
    ...
    try {
      ...
      queryError(error2);
      throw new Error("Failed to execute insert query in transaction " + error2);
    }
  }
  async massiveInsertQuery(query, params, typeofModel) {
    if (!this.pgClient) {
  ...
  async massiveUpdateQuery(query, params, typeofModel) {
    if (!this.pgClient) {
      throw new Error("PostgresTransaction not started.");
    }
    try {
  ...
  async massiveDeleteQuery(query, params, typeofModel) {
    if (!this.pgClient) {
      throw new Error("PostgresTransaction not started.");
    }
    try {
  ...
  async queryUpdate(query, params) {
    ...
    try {
      ...
      queryError(error2);
      throw new Error("Failed to execute update query in transaction " + error2);
    }
  }
  async queryDelete(query, params) {
    if (!this.pgClient) {
    ...
    try {
      ...
      queryError(error2);
      throw new Error("Failed to execute delete query in transaction " + error2);
    }
  }
  /**
   * Start transaction.
...
var SQLiteQueryBuilder = class _SQLiteQueryBuilder extends QueryBuilder {
  ...
  async one(options = { throwErrorOnNull: false }) {
    ...
    log(query, this.logs, this.params);
    try {
      const result = await this.promisifyQuery(query, this.params);
  ...
  promisifyQuery(query, params) {
    return new Promise((resolve, reject) => {
      this.sqLiteConnection.get(query, params, (err, result) => {
        if (err) {
          reject(err);
        }
        resolve(result);
...
var SQLiteUpdateQueryBuilder = class _SQLiteUpdateQueryBuilder extends ModelUpdateQueryBuilder {
   ...
   * @param isNestedCondition - A boolean indicating whether the query is nested in another query.
   */
  constructor(model, table, sqlLiteConnection, logs, isNestedCondition = false, sqlDataSource) {
    super(model, table, logs, false, sqlDataSource);
    this.joinQuery = "";
    this.isNestedCondition = false;
  ...
  promisifyQuery(query, params) {
    return new Promise((resolve, reject) => {
      this.sqlConnection.get(query, params, (err, result) => {
        if (err) {
          reject(err);
        }
        resolve(result);
...
var SQLiteDeleteQueryBuilder = class _SQLiteDeleteQueryBuilder extends ModelDeleteQueryBuilder {
  ...
  async execute(trx) {
    ...
    log(query, this.logs, this.whereParams);
    try {
      const result = await this.promisifyQuery(query, this.whereParams);
      return await parseDatabaseDataIntoModelResponse(
        result,
        this.model
      );
  ...
  async softDelete(options) {
    const {
      column: column2 = "deletedAt",
      value = import_luxon4.DateTime.local().toString(),
      trx
    } = options || {};
    let { query, params } = this.updateTemplate.massiveUpdate(
      [column2],
...
// src/Sql/Sqlite/SQLiteManager.ts
var SQLiteModelManager = class extends AbstractModelManager {
  ...
  async updateRecord(model, trx) {
    ...
    try {
      ...
      log(updateQuery.query, this.logs, updateQuery.params);
      return await this.promisifyQuery(
        updateQuery.query,
        updateQuery.params
      );
    } catch (error2) {
...
var Model = class {
  ...
  static delete() {
    const typeofModel = this;
    typeofModel.establishConnection();
    return typeofModel.sqlInstance.getModelManager(typeofModel).delete();
  }
  /**
   ...
   * @returns
   */
  static deleteByColumn(column2, value, trx) {
    const typeofModel = this;
    typeofModel.establishConnection();
    return typeofModel.sqlInstance.getModelManager(typeofModel).deleteByColumn(column2, value, trx);
  }
  /**
...
var RedisDataSource = class _RedisDataSource {
  ...
  static async connect(input) {
    ...
    try {
      ...
      throw new Error(`Failed to connect to Redis: ${error2}`);
    }
  }
  /**
  ...
  static async set(key, value, expirationTime) {
    ...
    try {
      ...
      throw new Error(`Failed to set key-value pair in Redis: ${error2}`);
    }
  }
  /**
  ...
  static async get(key) {
    try {
      ...
      throw new Error(`Failed to get value from Redis: ${error2}`);
    }
  }
  /**
  ...
  static async delete(key) {
    try {
      ...
      throw new Error(`Failed to delete key from Redis: ${error2}`);
    }
  }
  /**
  ...
  static async flushAll() {
    try {
      ...
      throw new Error(`Failed to flush Redis database: ${error2}`);
    }
  }
  /**
  ...
  static async disconnect() {
    try {
      ...
      throw new Error(`Failed to disconnect from Redis: ${error2}`);
    }
  }
  /**
   * @description Sets a key-value pair in the Redis database
...
(async () => {
  ...
  const value = 1;
  const user = await User.query().when(value, (value2, query) => {
    query.where("id", value2);
  }).one();
  console.log(user);
