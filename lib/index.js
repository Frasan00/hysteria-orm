...
var MysqlTransaction = class {
  ...
  async queryInsert(query, params, typeofModel) {
    ...
    const [savedModel] = await this.mysql.query(
      select,
      insertId
    );
  ...
  async massiveInsertQuery(query, params, typeofModel) {
    ...
    try {
      ...
      const [savedModels] = await this.mysql.query(select);
...
var _SqlDataSource = class _SqlDataSource extends DataSource {
  ...
  async startTransaction() {
    switch (this.type) {
      case "mariadb":
      case "mysql":
        const sqlConnection = await import_promise.default.createConnection({
          host: this.host,
          port: this.port,
          user: this.username,
        ...
        return trxMysql;
      case "postgres":
        const pgClient = new import_pg.default.Client({
          host: this.host,
          port: this.port,
          user: this.username,
          password: this.password,
...
var Model = class {
  ...
  static async first(options = { throwErrorOnNull: false }) {
    const typeofModel = this;
    typeofModel.establishConnection();
    return await typeofModel.sqlInstance.getModelManager(typeofModel).query().one(options);
  }
  /**
   ...
   * @returns {Promise<T | null>}
   */
  static insert(modelData, trx) {
    const typeofModel = this;
    typeofModel.establishConnection();
    return typeofModel.sqlInstance.getModelManager(typeofModel).insert(modelData, trx);
  }
  /**
   ...
   * @returns {Promise<T[]>}
   */
  static insertMany(modelsData, trx) {
    const typeofModel = this;
    typeofModel.establishConnection();
    return typeofModel.sqlInstance.getModelManager(typeofModel).insertMany(modelsData, trx);
  }
  /**
   ...
   */
  static async firstOrCreate(searchCriteria, createData, trx) {
    const typeofModel = this;
    typeofModel.establishConnection();
    const modelManager = typeofModel.sqlInstance.getModelManager(typeofModel);
    ...
    return await modelManager.insert(createData, trx);
  }
  /**
  ...
  static async upsert(searchCriteria, data, options = {
    ...
    if (doesExist) {
      ...
      if (options.updateOnConflict) {
        return await modelManager.updateRecord(data, options.trx);
      }
      return doesExist;
    }
    return await modelManager.insert(data, options.trx);
  }
  /**
  ...
  static async upsertMany(searchCriteria, data, options = {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (doesExist) {
        ...
        if (options.updateOnConflict) {
          results.push(
            await modelManager.updateRecord(record, options.trx)
          );
          continue;
        }
      ...
      results.push(await modelManager.insert(record, options.trx));
    }
    return results;
  }
  /**
