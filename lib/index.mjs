...
WHERE ${camelToSnakeCase(column)} ${operator} ${parseValue2(
      value
    )}`,
...
WHERE ${camelToSnakeCase(column)} != ${parseValue2(
      value
    )}`,
...
WHERE ${camelToSnakeCase(
    ...
    andWhereBetween: (column, min, max) => ` AND ${camelToSnakeCase(
      column
    )} BETWEEN ${min} AND ${max}`,
...
WHERE ${camelToSnakeCase(
    ...
    andWhereNotBetween: (column, min, max) => ` AND ${camelToSnakeCase(
      column
    )} NOT BETWEEN ${min} AND ${max}`,
    orWhereNotBetween: (column, min, max) => ` OR ${camelToSnakeCase(
      column
    )} NOT BETWEEN ${min} AND ${max}`,
...
var MysqlQueryBuilder = class extends QueryBuilder {
  ...
  join(relationTable, primaryColumn, foreignColumn) {
    const join = JOIN_default(this.tableName, relationTable, primaryColumn, foreignColumn);
