...
function modelFromSnakeCaseToCamel(model) {
  if (!model) {
    return null;
  }
  const camelCaseModel = {
    metadata: model.metadata,
    aliasColumns: model.aliasColumns
  };
  Object.keys(model).forEach((snakeCaseKey) => {
    const camelCaseKey = fromSnakeToCamelCase(snakeCaseKey);
    camelCaseModel[camelCaseKey] = model[snakeCaseKey];
  });
...
var updateTemplate = (table) => {
  return {
    update: (columns, values, primaryKey, primaryKeyValue) => {
      columns = columns.map((column) => camelToSnakeCase(column));
      return `UPDATE ${table} SET ${columns.map((column, index) => parseColumnValue(column, values[index])).join(", ")} WHERE ${primaryKey} = ${primaryKeyValue};`;
    }
  };
};
...
WHERE ${tableName}.${camelToSnakeCase(column)} = ${value} `,
    andWhere: (column, value, operator = "=") => ` AND ${tableName}.${camelToSnakeCase(column)} = ${value} `,
    orWhere: (column, value, operator = "=") => ` OR ${tableName}.${camelToSnakeCase(column)} = ${value} `,
    whereNot: (column, value) => `
WHERE ${tableName}.${camelToSnakeCase(column)} != ${value} `,
    andWhereNot: (column, value) => ` AND ${tableName}.${camelToSnakeCase(column)} != ${value} `,
    orWhereNot: (column, value) => ` OR ${tableName}.${camelToSnakeCase(column)} != ${value} `,
    whereNull: (column) => `
...
WHERE ${tableName}.${camelToSnakeCase(column)} IN (${values.join(
      ", "
    )}) `,
    andWhereIn: (column, values) => ` AND ${tableName}.${camelToSnakeCase(column)} IN (${values.join(
      ", "
    )}) `,
    orWhereIn: (column, values) => ` OR ${tableName}.${camelToSnakeCase(column)} IN (${values.join(", ")}) `,
    whereNotIn: (column, values) => `
WHERE ${tableName}.${camelToSnakeCase(column)} NOT IN (${values.join(
      ", "
    )}) `,
    andWhereNotIn: (column, values) => ` AND ${tableName}.${camelToSnakeCase(column)} NOT IN (${values.join(
      ", "
    )}) `,
    orWhereNotIn: (column, values) => ` OR ${tableName}.${camelToSnakeCase(column)} NOT IN (${values.join(
      ", "
    )}) `,
    rawWhere: (query) => `
WHERE ${query} `,
...
var MysqlModelManager = class extends AbstractModelManager {
  ...
  async find(input) {
    try {
      if (!input) {
        const select = SELECT_default(new this.model().metadata.tableName);
...
var PostgresModelManagerUtils = class {
  ...
  // Parses and fills input relations directly into the model
  async parseQueryBuilderRelations(model, input, pgPool, logs) {
    if (input.length === 0) {
      return;
    }
    if (!model.metadata.primaryKey) {
...
var PostgresModelManager = class extends AbstractModelManager {
  ...
  async update(model, trx) {
    ...
    try {
      const updateQuery = MySqlModelManagerUtils_default.parseUpdate(model);
      log(updateQuery, this.logs);
      const { rowCount } = await this.pgPool.query(updateQuery);
      return rowCount;
    } catch (error) {
      queryError(error);
      throw new Error("Query failed " + error);
  ...
  async deleteByColumn(column, value, trx) {
    ...
    try {
      ...
      log(deleteQuery, this.logs);
      const { rowCount } = await this.pgPool.query(deleteQuery);
      return rowCount;
    } catch (error) {
      queryError(error);
      throw new Error("Query failed " + error);
