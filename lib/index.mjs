...
var MysqlTransaction = class {
  ...
  async massiveUpdateQuery(query, params, selectQueryDetails) {
    ...
    try {
      ...
      const afterUpdateDataQuery = modelIds.length ? `SELECT * FROM ${table} ${joinClause} WHERE ${primaryKey} IN (${Array(
        modelIds.length
      ).fill("?").join(",")}) ` : `SELECT * FROM ${table}`;
...
var SQLiteDeleteQueryBuilder = class _SQLiteDeleteQueryBuilder extends ModelDeleteQueryBuilder {
  ...
  async delete(trx) {
    ...
    const preDeleteModels = await this.promisifyQuery(
      `SELECT * FROM ${this.table} ${this.joinQuery} ${this.whereQuery};`,
      this.whereParams
    );
...
var SQLiteTransaction = class {
  ...
  async massiveUpdateQuery(query, params, selectQueryDetails) {
    ...
    try {
      log(query, this.logs, params);
      const rows = await this.promisifyQuery(query, params);
      if (!rows.length) {
        return [];
      }
      const afterUpdateDataQuery = modelIds.length ? `SELECT * FROM ${table} ${joinClause} WHERE ${primaryKey} IN (${Array(
  ...
  promisifyQuery(query, params, typeofModel, sqliteConnection, sqlModelManagerUtils, options = {
    isCreate: false,
    isMassiveCreate: false,
    models: []
  }) {
