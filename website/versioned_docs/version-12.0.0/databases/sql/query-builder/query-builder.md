...
The `returning` array works across all supported databases. PostgreSQL, CockroachDB, SQLite, and MariaDB use the native `RETURNING`/`OUTPUT` clause. For MySQL, Hysteria ORM automatically performs a follow-up `SELECT` query to fetch the requested columns after the write operation.
:::
