...
Available `init` types: `sqlite`, `mysql`, `postgres`, `mariadb`, `cockroachdb`, `mssql`, `mongodb`, `redis`.
...
`generate:migrations` and `sync` work **only for PostgreSQL, MySQL, MariaDB, and CockroachDB**. On SQLite/MSSQL, write migrations by hand with `create:migration` and the `Schema` builder.
:::
