...
- Aliases are available directly integrated in the select method, for example select('new as newName') will generate an alias in the `$additional` property that every model instances has
```typescript
import { User } from "./User";
