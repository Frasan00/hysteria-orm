...
```typescript
import "reflect-metadata";
import { Model, Has_one, Has_many, column, hasOne, manyToMany, ModelQueryBuilder } from "hysteria-orm";
...
- Sql methods are available in the query builder, the results aliases will be available in the $additionalColumns property
...
// SQL Methods
// You can use sql methods directly in the query builder, the results will be available in the $additionalColumns property
const user: User | null = await User.query()
    .select("COUNT(*) as total")
    .where("name", "John Doe")
    .one();
const userCount = user?.$additionalColumns?.total;
...
- Aliases are available in the query builder, for example select('new as newName') will generate an alias in the $additionalColumns property that every model instances has
```typescript
import { User } from "./User";
...
## create:migration <name> [-j, --javascript]
