...
class MysqlSelectJsonInterpreter implements Interpreter {
  ...
  private buildExtractSql(column: string, jsonPath: JsonPath): string {
    const pathStr = jsonPath.toMysql();
  ...
  private buildExtractTextSql(column: string, jsonPath: JsonPath): string {
    ...
    if (pathStr === "$") {
      return `JSON_UNQUOTE(${column})`;
    }

    return `JSON_UNQUOTE(JSON_EXTRACT(${column}, '${pathStr}'))`;
  }
  ...
  private buildArrayLengthSql(column: string, jsonPath: JsonPath): string {
    ...
    return `JSON_LENGTH(${column}, '${pathStr}')`;
  }
  ...
  private buildObjectKeysSql(column: string, jsonPath: JsonPath): string {
    ...
    if (pathStr === "$") {
      return `JSON_KEYS(${column})`;
    }

    return `JSON_KEYS(${column}, '${pathStr}')`;
  }
...
export default new MysqlSelectJsonInterpreter();
