...
class OracleSelectJsonInterpreter implements Interpreter {
  ...
  private buildExtractSql(column: string, jsonPath: JsonPath): string {
    const pathStr = jsonPath.toOracle();
  ...
  private buildArrayLengthSql(column: string, jsonPath: JsonPath): string {
    ...
    return `JSON_QUERY(${column}, '${pathStr}.size()')`;
  }
  ...
  private buildObjectKeysSql(column: string, jsonPath: JsonPath): string {
    ...
    if (pathStr === "$") {
      return `JSON_QUERY(${column}, '$.keyNames()')`;
    }

    return `JSON_QUERY(${column}, '${pathStr}.keyNames()')`;
  }
...
export default new OracleSelectJsonInterpreter();
