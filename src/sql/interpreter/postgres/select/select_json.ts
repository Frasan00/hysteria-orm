...
class PostgresSelectJsonInterpreter implements Interpreter {
  ...
  private buildExtractSql(column: string, jsonPath: JsonPath): string {
    const pathSql = jsonPath.toPostgres(false);
    return pathSql ? `${column}${pathSql}` : column;
  }
  ...
  private buildExtractTextSql(column: string, jsonPath: JsonPath): string {
    ...
    const pathSql = jsonPath.toPostgres(true);
    return `${column}${pathSql}`;
  }
  ...
  private buildArrayLengthSql(column: string, jsonPath: JsonPath): string {
    ...
    return `jsonb_array_length(${extractedPath})`;
  }

  private buildObjectKeysSql(column: string, jsonPath: JsonPath): string {
    const extractedPath = this.buildExtractSql(column, jsonPath);
    return `jsonb_object_keys(${extractedPath})`;
  }
...
export default new PostgresSelectJsonInterpreter();
