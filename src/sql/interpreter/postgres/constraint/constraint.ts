  ...
  toSql(node: QueryNode): ReturnType<typeof AstParser.prototype.parse> {
    ...
    if (cNode.constraintType === "primary_key") {
      if (cNode.columns && cNode.columns.length > 0) {
        ...
        if (colsArr.length === 1) {
          return { sql: `${sqlPrefix}primary key`, bindings: [] };
        }
        const cols = colsArr
          .map((c) => utils.formatStringColumn("postgres", c))
          .join(", ");
    ...
    if (cNode.constraintType === "unique") {
      if (cNode.columns && cNode.columns.length > 0) {
        ...
        const sqlPrefixU = cNode.constraintName
          ...
          : "";
        if (colsArr.length === 1) {
          // column-level unique does not need column list
          return { sql: `${sqlPrefixU}unique`, bindings: [] };
        }
        const cols = colsArr
          .map((c) => utils.formatStringColumn("postgres", c))
          .join(", ");
