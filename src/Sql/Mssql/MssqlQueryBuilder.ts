...
import { MssqllModelManager } from "./MssqlModelManager";

export class MssqlQueryBuilder<T extends Model> extends QueryBuilder<T> {
  ...
  public constructor(
    model: typeof Model,
    table: string,
    sqlConnection: mssql.ConnectionPool,
    logs: boolean,
  ...
  public async many(options: ManyOptions = {}): Promise<T[]> {
    ...
    try {
      ...
      const result = (await mssqlRequest.query(query)).recordsets as any[];
  ...
  public async getCount(
    ...
    if (options.ignoreHooks) {
      const [result]: any = await this.sqlConnection.query(
  ...
  // SELECT
  public select(...columns: string[]): MssqlQueryBuilder<T>;
  public select(...columns: (SelectableType<T> | "*")[]): MssqlQueryBuilder<T>;
  public select(
  ...
  public addRelations(relations: RelationType<T>[]): MssqlQueryBuilder<T> {
  ...
  public whereBuilder(cb: (queryBuilder: MssqlQueryBuilder<T>) => void): this {
    const queryBuilder = new MssqlQueryBuilder(
    ...
    cb(queryBuilder as unknown as MssqlQueryBuilder<T>);
  ...
  public orWhereBuilder(
    cb: (queryBuilder: MssqlQueryBuilder<T>) => void,
  ): this {
    const nestedBuilder = new MssqlQueryBuilder(
      this.model as typeof Model,
      this.table,
      this.sqlConnection,
      this.logs,
    ...
    cb(nestedBuilder as unknown as MssqlQueryBuilder<T>);
  ...
  public copy(): ModelQueryBuilder<T> {
    const queryBuilder = new MssqlQueryBuilder<T>(
      this.model as typeof Model,
      this.table,
      this.sqlConnection,
      this.logs,
  ...
  protected groupFooterQuery(): string {
    return (
      this.groupByQuery + this.orderByQuery + this.offsetQuery + this.limitQuery
    );
  }
}
